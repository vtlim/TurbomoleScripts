...
# dirSpecs, botSpecs, entries, defInp
# 
...
def title(dirSpecs, botSpecs, entries, defInp):
   key='$title'
...
def readCoord(dirSpecs, botSpecs, entries, defInp):
   key='$coord'

   coord=getLine(entries, key)
   coordBot=getLine(botSpecs, key)
   coordDir=getLine(dirSpecs, key)
...
def assignSym(dirSpecs, botSpecs, entries, defInp):
   key='$sym'

   sym=getLine(entries, key)
   symBot=getLine(botSpecs, key)
   symDir=getLine(dirSpecs, key)
...
def detInternals(dirSpecs, botSpecs, entries, defInp):
   key='$internal'

   internal=getLine(entries, key)
   internalBot=getLine(botSpecs, key)
   internalDir=getLine(dirSpecs, key)
...
def assignFrags(dirSpecs, botSpecs, entries, defInp):
   key='$frag'

   frag=getLine(entries, key)
   fragBot=getLine(botSpecs, key)
   fragDir=getLine(dirSpecs, key)
...
def defBasis(dirSpecs, botSpecs, entries, defInp):
   key='$basis'

   basis=getLine(entries, key)
   basisBot=getLine(botSpecs, key)
   basisDir=getLine(dirSpecs, key)
...
def useHcore(dirSpecs, botSpecs, entries, defInp):
   key='$hcore'

   hcore=getLine(entries, key)
   hcoreBot=getLine(botSpecs, key)
   hcoreDir=getLine(dirSpecs, key)
...
def eht(dirSpecs, botSpecs, entries, defInp):
   key='$eht'

   eht=getLine(entries, key)
   ehtBot=getLine(botSpecs, key)
   ehtDir=getLine(dirSpecs, key)
...
def molCharge(dirSpecs, botSpecs, entries, defInp):
   key='$charge'

   charge=getLine(entries, key)
   chargeBot=getLine(botSpecs, key)
   chargeDir=getLine(dirSpecs, key)
...
#
#KeyFormat: $occ [int]
def setOcc(dirSpecs, botSpecs, entries, defInp):
   key='$occ'

   occ=getLine(entries, key)
   occBot=getLine(botSpecs, key)
   occDir=getLine(dirSpecs, key)
...
def dft(dirSpecs, botSpecs, entries, defInp):
   key='$dft'

   dft=getLine(entries, key)
   dftBot=getLine(botSpecs, key)
   dftDir=getLine(dirSpecs, key)
...
def ri(dirSpecs, botSpecs, entries, defInp):
   key='$ri'

   ri=getLine(entries, key)
   riBot=getLine(botSpecs, key)
   riDir=getLine(dirSpecs, key)
...
#               denconv=[float]
def cc(dirSpecs, botSpecs, entries, defInp):
   key='$cc'

   cc=getLine(entries, key)
   ccBot=getLine(botSpecs, key)
   ccDir=getLine(dirSpecs, key)
...
def rirpa(dirSpecs, botSpecs, entries, defInp):
   key='$rirpa'

   rirpa=getLine(entries, key)
   rirpaBot=getLine(botSpecs, key)
   rirpaDir=getLine(dirSpecs, key)
...
def scf(dirSpecs, botSpecs, entries, defInp):
   key='$scf'

   scf=getLine(entries, key)
   scfBot=getLine(botSpecs, key)
   scfDir=getLine(dirSpecs, key)
